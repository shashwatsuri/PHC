...
class HumanoidIm(humanoid_amp_task.HumanoidAMPTask):
    ...
    def post_physics_step(self):
        ...
        if flags.im_eval:
            ...
            a = torch.cat((body_pos[0,:self.cfg.joint_id_noise], body_pos[0,self.cfg.joint_id_noise+1:])).unsqueeze(0)
            b = torch.cat((motion_res['rg_pos'][0,:self.cfg.joint_id_noise], motion_res['rg_pos'][0,self.cfg.joint_id_noise+1:])).unsqueeze(0)
            self.extras['mpjpe'] = (a-b).norm(dim=-1).mean(dim=-1)
